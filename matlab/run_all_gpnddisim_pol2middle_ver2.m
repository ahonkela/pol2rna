...
load series_pol2middle_ver1.mat
...
for k=kstart:kend,
  ...
  if (sum(isnan(dataVals1))>0) || (sum(isnan(dataVals2))>0), continue; end;
  ...
  [jointmodelb,jointtransforminfo,pol2modelb,rnamodelb,naive_ll,rbf_ll,joint_ll]=createNdGeneGPModels_celltimes({temptimes,temptimes},{tempvals1,tempvals2},lengthscale,maxiters,ninits,[],[]);
  ...
  if mod(k,5)==0,
    tempfilename=sprintf('fittingresults_pol2middle_startpercent%d.mat',startpercent);
