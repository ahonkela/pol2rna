...
for k=kstart:kend,
  ...
  [pars1,nams1]=gpdisimExtractParam(jointmodelb)
  parst1=transformParametersWithSettings(pars1,jointtransforminfo,'atox');  
  parst2=parst1;
  parst2(11)=mattisparameters(k,1); % RNA start mean
  parst2(10)=mattisparameters(k,2); % Basal rate
  parst2(5)=mattisparameters(k,3)^2; % DISIM-level variance = square of sensitivity
  parst2(4)=mattisparameters(k,4); % DISIM-level decay
  parst2(7)=mattisparameters(k,5); % DISIM-level delay
  parst2(12)=jointmodelb.y(1); % POL2 start mean
  parst2(8)=1e-3; % start with low POL2 noise assumption
  parst2(9)=1e-3; % start with low RNA noise assumption
  pars2=transformParametersWithSettings(parst2,jointtransforminfo,'xtoa');
  model2=gpdisimExpandParam(jointmodelb,pars2);
  model2_ll=gpdisimLogLikelihood(model2);
  ...
  model3_ll=gpdisimLogLikelihood(model3);
