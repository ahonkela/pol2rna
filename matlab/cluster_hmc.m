...
filenames2 = filenames(Ifiles);

npredtimes=500;
pol2preds=zeros(length(goodids),npredtimes);
rnapreds=zeros(length(goodids),npredtimes);
for i=1:length(goodids),
  Ithisgene=find(ids==goodids(i));
  for k=1:length(Ithisgene),
    ifile=filenames{Ithisgene(k)};

    load([resultdir ifile]);
    for l=1:size(HMCsamples,1),
      [priormeans,posteriormeans,covmatrix,rbfposteriormeans,rbfcovmatrix] = gpnddisimPredict(model,predtimes,predict_rna);
    end;
