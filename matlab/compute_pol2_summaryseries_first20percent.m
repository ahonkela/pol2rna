...
for k=1:size(bininfo,1),
  ...
  if ~isnan(firstexonstarts(k)),
    ...
    % start of first exon + 20% towards the end
    differencefromend=floor(0.8*(bininfo(k,3)-firstexonstarts(k)));
    ...
    % start of the gene + 20% towards the end
    differencefromend=floor(0.8*(bininfo(k,3)-bininfo(k,2)));
    if differencefromend < -200,
      nextexonbins(k)=-100;
    elseif differencefromend < 0,
      nextexonbins(k)=0;
