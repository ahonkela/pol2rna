...
Dvals_base = log(2) ./ [2 4 8 16 32 64]; %[0.32, 0.16, 0.08, 0.04, 0.02, 0.01];
Dvals = cell(1, length(Dvals_base)*4);
for l=1:length(Dvals_base),
  Dvals{0*length(Dvals_base)+l} = [80, Dvals_base(l), Dvals_base(l)/2];
  Dvals{1*length(Dvals_base)+l} = [80, Dvals_base(l), Dvals_base(l)/1.5];
  Dvals{2*length(Dvals_base)+l} = [80, Dvals_base(l), Dvals_base(l)*2];
  Dvals{3*length(Dvals_base)+l} = [80, Dvals_base(l), Dvals_base(l)*1.5];
end
Deltavals = [0 4 8 12];
...
save simulated_data_changingD.mat pol2data rnadata Dvals Deltavals t_gen
