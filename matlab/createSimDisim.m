function [gpsim3model,transforminfo]=createSimDisim(timevector,dataVals1,dataVals2,lengthscale,initializationtype);
%,inversesquarewidth0,pol2effectvar0,rnadecay0,pol2noisevar0,rnanoisevar0,rnabasal0);

fprintf(1,'createSimDisim step 1\n');
...
if ~isempty(dataVals2),
  % dataVals2=dataVals2-dataVals2(1);
  % dataVals2=dataVals2/sqrt(var(dataVals2));
  ...
  %dataVals2=dataVals2-min(dataVals2);
end;
...
fprintf(1,'createSimDisim step 2\n');
...
if numgenes>0,
  ...
  pol2effectvar_index=3;
  rnadecay_index=4;
  rnaeffectvar_index=5;
  rbfvar_index=6;
  rnadelay_index=7;
  pol2noisevar_index=8;
  rnanoisevar_index=9;
  rnabasal_index=10;
  rnastartmean_index=11;
  pol2mean_index=12;
else
  pol2decay_index=1;
  inversewidth_index=2;
  pol2effectvar_index=3;
  pol2noisevar_index=4;
  rnadecay_index=nan;
...
fprintf(1,'createSimDisim step 3\n');


% set parameter ranges
pol2decay_range=[0 100];
inversewidth_range=1./([1280 5].^2);

pol2effectvar_range=[0.0002 2]*var(dataVals1);
...
rnadecay_range=[0.00000001 1.5];
rnaeffectvar_range=[0 2]; % *var(dataVals2)/var(dataVals1);
rbfvar_range=[0 10];
...
transformsettings={};
if(~isnan(pol2decay_index)), transformsettings{pol2decay_index}=pol2decay_range; end;
...
if(~isnan(rbfvar_index)), transformsettings{rbfvar_index}=rbfvar_range; end;
...
transforminfo.indices=[pol2decay_index inversewidth_index pol2effectvar_index rnadecay_index rnaeffectvar_index rbfvar_index rnadelay_index pol2noisevar_index rnanoisevar_index rnabasal_index rnastartmean_index pol2mean_index];

transforminfo.settings={pol2decay_range,inversewidth_range,pol2effectvar_range,rnadecay_range,rnaeffectvar_range,rbfvar_range,rnadelay_range,pol2noisevar_range,rnanoisevar_range,rnabasal_range,rnastartmean_range,pol2mean_range};


fprintf(1,'createSimDisim step 4\n');


%transformsettings
%pause

% length scale, here fixed to starting value...
% options.fix.index(1)=inversewidth_index;    
% if isempty(lengthscale),
%   % initialization of inverse squared width
%   options.fix.value(1)=sigmoidabTransform(1/(2*(15^2)), 'xtoa', inversewidth_range);
% else
%   options.fix.value(1)=sigmoidabTransform(1/(2*(lengthscale^2)), 'xtoa', inversewidth_range);
% end;

% decay of f to p, here fixed to 0
options.fix.index(1)=pol2decay_index;
options.fix.value(1)=sigmoidabTransform(exp(-20), 'xtoa', pol2decay_range);

if numgenes>0,
  % rbf variance, unidentifiable, here fixed to 1
  options.fix.index(2)=rbfvar_index;     
  options.fix.value(2)=sigmoidabTransform(1, 'xtoa', rbfvar_range);  
end;
  
fprintf(1,'createSimDisim step 5\n');


gpsim3model = gpasimTemp3Create(numgenes, 1, timevector, dataValsMatrix,dataVarsMatrix, options, annotation, transformsettings );


fprintf(1,'createSimDisim step 6\n');
...
if(initializationtype==1),
  ...
  if numgenes>0,
    ...
    rnastartmean_initval=dataVals2(1);
...
if(initializationtype>3),
  ...
  for itrial=1:ntrials,
    ...
    templl=gpdisimLogLikelihood(gpsim3model)
    if templl>bestll,
      bestpars=pars;
      bestll=templl;
    end;
...
[pars,name]=gpdisimExtractParam(gpsim3model);
%nams
%pars
