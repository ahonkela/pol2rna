function [difftotalgradient,modeltotalgradient]=testModelGradientMeanOnly(model,updatekernel,updatemean);
...
for k=1:length(pars),
  ...
  valtemp1=0.5*(-dim*log(2*pi) - logDetK - model.m'*invK*model.m);
  
  parstemp(k)=pars(k)+myeps;
  modeltemp=gpdisimExpandParam(model,parstemp,1,1);
  valtemp2=0.5*(-dim*log(2*pi) - logDetK - model.m'*invK*model.m);
