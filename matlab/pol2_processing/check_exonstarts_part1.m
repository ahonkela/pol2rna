...
A=A.A;
tcountsfile=read_stringfile('transcript_counts.txt',[' ' ',' 9 10 13]); % space comma tab linefeed carriagereturn
...
for i=1:length(tcountsfile),
  ...
  tcounts(i,4)=str2double(tcountsfile{i}{4}); % transcript length
end;

% Find all transcripts with at least 1.1. reads on average
Itr = find(tcounts(:,3)>=1.1);
tcounts_active = tcounts(Itr,:);

% Find all genes where there are active transcripts
geneids=unique(tcounts_active(:,1));
...
for k=1:length(geneids),
  ...
  gid=geneids(k);
  Itrids=find(tcounts_active(:,1)==gid);
  trids=tcounts_active(Itrids,2);
