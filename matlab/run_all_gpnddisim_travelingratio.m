...
cd(pol2dir)
load pol2series_first20percent.mat
pol_summaryseries=pol2_summaryseries_middle;
rna_summaryseries=pol2_summaryseries_end;
...
for k=kstart:kend,
  ...
  if (sum(isnan(dataVals1))==0) && (sum(isnan(dataVals2))==0),
    ...
    tempvals2=tempvals2-min(tempvals2);
    if mean(tempvals2.^2)>0,
      tempvals2=tempvals2/sqrt(mean(tempvals2.^2));
    end;
    
    maxiters=100; ninits=8; lengthscale=5;
    [jointmodelb,jointtransforminfo,pol2modelb,rnamodelb,naive_ll,rbf_ll,joint_ll]=createNdGeneGPModels(temptimes,tempvals1,tempvals2,lengthscale,maxiters,ninits,[],0);
  ...
  if mod(k,5)==0,
    tempfilename=sprintf('fittingresults_travelingratio_temp_%d.mat',startpercent);
